...
//		if(mc->options.getBooleanValue(OPTIONS_FANCY_GRAPHICS)) {
			...
			glFogf(GL_FOG_START, renderDistance  * 0.15f);
			glFogf(GL_FOG_END, renderDistance *0.70f);
...
void GameRenderer::setupFog(int i) {
	...
	if (player->isUnderLiquid(Material::water)) {
		...
		glFogf(GL_FOG_DENSITY, 0.08f); // was 0.06
		...
		glFogf(GL_FOG_START, renderDistance * 0.35f);
		glFogf(GL_FOG_END, renderDistance * 0.85f);
		if (i < 0) {
...
void GameRenderer::setupClearColor(float a) {
    ...
    // some stuff
// hmmm
long worldTime = level->getTime() % 24000;

// more stuff :D
float sunsetBlend = 0.0f;
if (worldTime > 11500 && worldTime < 13000) {
    // oooooooo
    sunsetBlend = 1.0f - std::abs((worldTime - 12250.0f) / 750.0f);
} else if (worldTime > 22500 || worldTime < 500) {
    // noice
    float t = (worldTime > 12000) ? worldTime - 24000 : worldTime;
    sunsetBlend = 1.0f - std::abs(t / 750.0f);
}
sunsetBlend = std::max(0.0f, std::min(1.0f, sunsetBlend));

// hmmmmm
// cool stuff heheh
fr = fr + (1.0f  - fr) * sunsetBlend * 0.30f;
fg = fg + (0.55f - fg) * sunsetBlend * 0.30f;
fb = fb + (0.10f - fb) * sunsetBlend * 0.30f;
